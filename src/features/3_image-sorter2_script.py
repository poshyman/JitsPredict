...
class ImageGui:
    ...
    def vote_csv(self,label):
        ...
        root, file_name = os.path.split(df.sorted_in_folder[self.index])
        output_path = os.path.join(root, 'Tagged', file_name[:-4]+".csv")
    ...
    def _load_image(path):
        ...
        for i in range(len(df)):
            # get coordinates
            y1, x1, y2, x2 = df['ymin'].iloc[i], df['xmin'].iloc[i], df['ymax'].iloc[i], df['xmax'].iloc[i]
            # Blue color in BGR
            color = (255, 0, 0)
            ...
            text = "P" + df['Label'].iloc[i][-2:] + " " + str(df['Score'].iloc[i])[:4]
            #Define font
    ...
    def _move_image(label, ind):
        ...
        input_label_file = df.sorted_in_folder[ind][:-4]+'_label_info.csv'
        output_label_file = output_path[:-4]+'_label_info.csv'
